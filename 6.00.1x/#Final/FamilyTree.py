...
class Family(object):
    ...
    def cousin(self, a, b):
        ...
        if a == b:
            #the darn strings are the same
            return (-1,0)
            
        anode = self.names_to_nodes[a]
        bnode = self.names_to_nodes[b]
        
        ##print "The A string is", a
        ##print "The B string is", b
        ##print "The A node is", anode
        ##print "The B node is", bnode
        
        #find all ancestors of a
        ancestorsOfA = []

        temp = anode.get_parent()
        ##print "A's direct parent is:", temp
        while not (temp == None):
            ##print "Adding A's ancestor:", temp
            ancestorsOfA.append(temp)
            temp = temp.get_parent()
        
        ##print "All of A's ancestors have been found. Total of ", len(ancestorsOfA)
        #find all ancestors of b
        ancestorsOfB = []
        
        temp = bnode.get_parent()
        ##print "B's direct parent is:", temp       
        while not (temp == None):
            ##print "Adding A's ancestor:", temp
            ancestorsOfB.append(temp)
            temp = temp.get_parent()
        ...
        if anode.parent == None:
            #a is at the top
            return (-1,len(ancestorsOfB))

        if bnode.parent == None:
            #b is at the top
            return (-1,len(ancestorsOfA))
        
        if anode in ancestorsOfB:
            #print "we're in here1"
            distance = 1
            for ancestor in ancestorsOfB:
                if not (anode == ancestor):
                    distance += 1
                else:
                    break
            return (-1,distance)

        if bnode in ancestorsOfA:
            #print "we're in here2"
            distance = 1
            for ancestor in ancestorsOfA:
                if not (bnode == ancestor):
                    distance += 1
                else:
                    break            
            return (-1,distance)
        ...
        ##print "All of B's ancestors have been found. Total of ", len(ancestorsOfB)

        #find the ancestor in common
        if len(ancestorsOfA) <= len(ancestorsOfB):
            for eachAncestorOfA in ancestorsOfA:
                if eachAncestorOfA in ancestorsOfB:
                    commonDude = eachAncestorOfA
                    break
        else:
            for eachAncestorOfB in ancestorsOfB:
                if eachAncestorOfB in ancestorsOfA:
                    commonDude = eachAncestorOfB
                    break

        ##print "the ancestor in common is", commonDude



        #find distance from a to common ancester
        d1 = 0
        for ancestor in ancestorsOfA:
            if not (ancestor == commonDude):
                d1 += 1
            else:
                break

        #print "Distance from A to common ancestor was", d1
        
        #find distance from b to common ancester
        d2 = 0
        for ancestor in ancestorsOfB:
            if not (ancestor == commonDude):
                d2 += 1
        ...
        #print "Distance from B to common ancestor was", d2
        
        #find the smalller of the two to return it
        ctype = min(d1,d2)
        #find the difference in distances
        cdegrees = max(d1,d2) - min(d1,d2)  
        #final return
        return (ctype, cdegrees)   
...
t, r = f.cousin("p", "c")
print "'p' is a", words[t], "cousin", r, "removed from 'c'"